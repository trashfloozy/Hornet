$hkSavePath = "$env:USERPROFILE\AppData\LocalLow\Team Cherry\Hollow Knight\"
$hkBackupPath = "$env:USERPROFILE\Desktop\Steam Backup Save\"

$emergencySave = "$env:USERPROFILE\Desktop\Steam Backup Save\EMERGENCY\Hollow Knight\"

$hkBackupedPath = "$env:USERPROFILE\Desktop\Steam Backup Save\Hollow Knight\"
$hkRestorePath = "$env:USERPROFILE\AppData\LocalLow\Team Cherry\"

function CopyBackup {
  [CmdletBinding()]
  param ()
  process {
    if (!(Test-Path -Path $hkBackupPath)) {
      New-Item -ItemType Directory -Path $hkBackupPath > $null
    }
    
    Copy-Item -Path "$hkSavePath" -Destination $hkBackupPath -Recurse -Force
  ...
  process {
    ...
    if (!(Test-Path -Path $emergencySave)) {
      ...
      switch ($emergencyInput.ToLower()) {
        {($_ -eq "y") -or ($_ -eq "yes")} {
          Copy-Item -Path "$hkSavePath" -Destination $emergencySave -Recurse -Force
...
function RestoreBackup {
  ...
  param ()
  process {
    if (!(Test-Path -Path $hkBackupedPath)) {
    ...
    if (Test-Path -Path $hkSavePath) {
      Remove-Item -Path $hkSavePath -Recurse -Force
    }
      
    Copy-Item -Path "$hkBackupedPath" -Destination $hkRestorePath -Recurse -Force
...
function emergencySecurity {
  if (!(Test-Path -Path $emergencySave)) {
    ...
    if (!(Test-Path -Path $hkSavePath)) {
      ...
      Copy-Item -Path "$hkSavePath" -Destination "$emergencySave\Hollow Knight" -Recurse -Force
      Write-Host -Object '> Saved Emergency Backup' -ForegroundColor 'Green'
    }
  }
}
